...
(defvar erl-trace-options
  (let (opts)
    (dolist (key erl-trace-option-keys opts)
      (setq opts (plist-put opts key
                            (if (memq key '(:location :newline)) t nil)))))
  "Plist storing current erl-trace configuration.")


(defun erl-get-version ()
  "Return the current Erlang/OTP version as an integer, e.g., 25."
  (string-to-number
   (string-trim
...
(defvar erl-trace-prefix
  (or (getenv "USER") "unknownuser")
  "Prefix used in trace logs. Defaults to the current USER
  environment variable or 'unknownuser' if unset.")
...
(defun erl-trace-key-bindings ()
  (define-key erlang-mode-map (kbd "C-c C-t") 'erl-trace-insert)
  (define-key erlang-mode-map (kbd "C-c C-e") 'erl-trace-run-cmd)
  (define-key erlang-mode-map (kbd "C-c C-r") 'erl-trace-store))

(add-hook 'erlang-mode-hook #'erl-trace-key-bindings)
